...
console.log('hi');
var element = dom.create('   <tr><td>后面的节点</td></tr> ');
var element1 = dom.create('   <tr><td>前面的节点</td></tr> ');
var element2 = dom.create('   <tr><td>追加一个节点</td></tr> ');
var wrapper = dom.create('   <div id="newWrapper">新爸爸包在外面呢<div>111</div></div> ');
dom.after(test, element); // 页面上的test

dom.before(test, element1);
dom.append(test.parentNode, element2);
dom.wrap(test1, wrapper);
var arr = dom.empty(newWrapper);
var arr1 = dom.empty(empty);
...
var styleValue = dom.style(test, 'border'); // 读取

console.log(styleValue);
dom.class.add(test, 'red'); // dom.class.remove(test,'red');

var bool = dom.class.contains(test, 'red');
console.log(bool); //事件函数绑定与移除

var fn = function fn() {
  console.log('点击了');
};

dom.on(test2, 'click', fn); // dom.off(test2,'click',fn)

var testDiv = dom.find('.test888');
console.log(testDiv);
var testDiv1 = dom.find('.test888', test1000);
console.log(testDiv1);
console.log(dom.parent(test));
console.log(dom.siblings(test999));
console.log(dom.next(testDiv1[0]));
var t = dom.find('#travel')[0];
dom.each(dom.children(t), function (item) {
  return dom.style(item, 'color', 'red');
});
